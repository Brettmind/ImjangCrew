    ...
    <section id="pricing" className="bg-background py-24 px-6">
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-16">
          ...
          {/* Toggle */}
          <div className="inline-flex items-center gap-3 p-1.5 rounded-full bg-card/50 border border-border/50">
            <button
              onClick={() => setYearly(false)}
              className={`px-5 py-2 rounded-full text-sm font-medium transition-all ${!yearly ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:text-foreground'}`}
            >
              월간
            </button>
            <button
              onClick={() => setYearly(true)}
              className={`px-5 py-2 rounded-full text-sm font-medium transition-all flex items-center gap-2 ${yearly ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:text-foreground'}`}
            >
              연간
            ...
            return (
              <div
                ...
                className={`relative flex flex-col rounded-2xl p-8 border transition-all duration-300 ${
                  p.highlight
                    ? 'border-border bg-card/50 backdrop-blur-sm shadow-xl shadow-primary/10'
              ...
              >
                ...
                <div className="mb-8">
                  <div className="flex items-baseline gap-1">
                    ...
                    {price > 0 && <span className="text-muted-foreground/60 text-sm">/월</span>}
                  </div>
                  {yearly && price > 0 && (
                    <p className="text-primary text-xs mt-1">연간 결제 시 ₩{(price * 12).toLocaleString()}</p>
                  )}
                </div>
                ...
                <button
                  className={`w-full py-3 rounded-full font-semibold text-sm transition-all mb-8 ${
                    p.highlight
                      ...
                      : 'border border-border text-primary hover:bg-card/80'
                  }`}
                >
                  {p.cta}
                </button>
                ...
                <div className="flex-1 space-y-3">
                  ...
                  {p.notIncluded.map((f) => (
                    <div key={f} className="flex items-start gap-3 text-sm">
                      ...
                      <span className="text-foreground/20">{f}</span>
                    </div>
                  ))}
                </div>
              </div>
